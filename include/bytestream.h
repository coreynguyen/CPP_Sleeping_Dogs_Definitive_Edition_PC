...
namespace bits { // bitwise function which mimic the maxscript api
...
class bytestream {
  public:
	...
	char* stream;
	bool islilEndian;
	bool ownsMemory; // Indicates if this instance owns the memory
    ...
    void assign(char* newStream, size_t newSize);
	size_t tell ();
    ...
    void asDDS (char* &buf, unsigned int width, unsigned int height, unsigned int mipCount = 1, std::string type = "DXT1");
	uint8_t readUbyte ();
	uint16_t readUshort ();
	uint32_t readUlong ();
	...
	void writelonglong (signed long long val);

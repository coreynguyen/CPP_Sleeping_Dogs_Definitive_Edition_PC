...
Data\Animation\%s.bin
Data\AnimationNIS\%s.bin
Data\AnimationNIS\LightRigs\%s
Data\CheckedIn\Common\Data\World\%s
Data\Gameplay\%s\%s.bin
data\gameplay\%s\uniqueuids.bin
Data\Global\%s.perm.bin
Data\Global\Act\%s.rig2
Data\Global\Act\AI\%s
Data\HK Autosave%s Slot
Data\HK GameSlot%s %d
Data\HK Photo%s %d
Data\Parts\%s.bin
Data\Parts\%s.perm.bin
Data\Parts\HD\%s.perm.bin
Data\Parts\LD\%s.perm.bin
Data\Parts\SD\%s.perm.bin
Data\UI\%s.perm.bin
Data\UI\Localization\%s
Data\World\%s\AISidewalkGraph
Data\World\%s\MetaGame
Data\World\%s\Region.perm.bin
Data\World\%s\Section\%s\%s\%sLOD.perm.bin
Data\World\%s\Section\%s\%s\%sSTD.perm.bin
Data\World\%s\Section\%s\%s\%sXXX.perm.bin
data\world\%s\terrain\terrainnode
...
std::unordered_map<uint32_t, std::string> fileHashMap;
...
// Provided CRC-32 table (define your precomputed table here)
...
// Function to hash strings using the provided CRC-32 table
uint32_t StringHashUpper32(const char* p_Str, uint32_t p_PrevHash) {
...
void initializeFileList() {
    std::string exePath = sysinfo::currentdir();
    std::string fileListPath = exePath + "\\filelist.txt";

    if (GetFileAttributes(fileListPath.c_str()) != INVALID_FILE_ATTRIBUTES) {
        loadFileList(fileListPath);
        std::cout << "File list loaded successfully from " << fileListPath << std::endl;
    } else {
        std::cout << "filelist.txt not found in the executable directory." << std::endl;
    }
}
...
void loadFileList(const std::string& filePath) {
    ...
    while (std::getline(file, line)) {
        ...
        // Remove comments after a semicolon
        size_t commentPos = line.find(';');
        if (commentPos != std::string::npos) {
            line = line.substr(0, commentPos);
        }

        // Trim the line again after removing comments
        line.erase(0, line.find_first_not_of(" \t\n\r"));
        line.erase(line.find_last_not_of(" \t\n\r") + 1);

        if (!line.empty()) {
            uint32_t hash = StringHashUpper32(line.c_str());
            fileHashMap[hash] = line;
        }
    }
}

std::string getFilePathFromHash(uint32_t hash) {
    auto it = fileHashMap.find(hash);
...
uint32_t getHashFromFilename(const std::string &filename) {
    ...
    // Check if the filename is a hex string of 8 or fewer characters
    ...
    // If not a hex string, use StringHashUpper32
    return StringHashUpper32(baseName.c_str());
    }
